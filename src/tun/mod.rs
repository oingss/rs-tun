...
impl TunInbound {
    ...
    /// sing-tun 风格构造：注入一个 [`crate::handler::Handler`] 而不是裸 channel。
    ///
    /// 引擎内部仍然以 channel 驱动（未改动，行为与 [`TunInbound::new`] 完全一致），
    /// 这里只是启动两个（启用 DNS 劫持时是三个）后台桥接任务，把 channel 收到的
    /// 值转发给 `handler` 的对应方法，参见 [`crate::handler::spawn_handler_bridge`]。
    pub fn with_handler(
        config: TunInboundConfig,
        handler: std::sync::Arc<dyn crate::handler::Handler>,
        dns_hijack: bool,
    ) -> Self {
        let (tcp_tx, udp_tx, dns_tx) =
            crate::handler::spawn_handler_bridge(handler, dns_hijack);
        Self {
            config,
            tcp_tx,
            udp_tx,
